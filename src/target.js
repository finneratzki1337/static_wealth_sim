import { DEFAULTS, TARGET_DEFAULTS, SOURCES, ASSUMPTIONS } from "./data/defaults.js";
...
import { formatCurrencyCompact } from "./ui/format.js";
...
function validateScenario({ targetNetMonthly, targetMode, payoutMode, endAge, baseParams }) {
  ...
  if (baseParams.annualReturnPre < 0) warnings.push("Annual return is negative.");
  if (baseParams.inflationAnnual < 0) warnings.push("Inflation is negative.");
...
function runSimulation() {
  try {
    ...
    const simulationParams = {
      ...
      targetNetWithdrawal: scenario.payoutMode === "untilAge"
        ? computeTargetNominal({
          targetNetMonthly: scenario.targetNetMonthly,
          targetMode: scenario.targetMode,
    ...
    reader.onload = () => {
      try {
        ...
        applyScenarioToInputs({
          ...DEFAULTS,
          ...TARGET_DEFAULTS,
          targetNetMonthly: isFiniteNumber(parsed.targetNetMonthly)
            ? parsed.targetNetMonthly
            : TARGET_DEFAULTS.targetNetMonthly,
